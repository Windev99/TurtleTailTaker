...
import com.wintereur.turtletail.taker.utils.DonationLinkHelper;
 ...
 * YoutubeChannelTaker.java is part of TurtleTail.
 *
...
public class YoutubeChannelTaker extends ChannelTaker {
    private static final String CHANNEL_FEED_BASE = "https://www.youtube.com/feeds/videos.xml?channel_id=";
    private static final String CHANNEL_URL_PARAMETERS = "/videos?view=0&flow=list&sort=dd&live_view=10000";

    private Document doc;

    public YoutubeChannelTaker(StreamingService service, ListLinkHandler urlIdHandler) {
    ...
    public InfoItemsPage<StreamInfoItem> getPage(String pageUrl) throws IOException, ExtractionException {
        ...
        JsonObject ajaxJson;
        try {
            ajaxJson = JsonParser.object().from(TurtleTail.getDownloader().download(pageUrl));
    ...
    private void collectStreamsFrom(StreamInfoItemsCollector collector, Element element) throws ParsingException {
        ...
        for (final Element li : element.children()) {
            if (li.select("div[class=\"feed-item-dismissable\"]").first() != null) {
                collector.commit(new YoutubeStreamInfoItemTaker(li) {
                    @Override
                    public String getUrl() throws ParsingException {
                        try {
